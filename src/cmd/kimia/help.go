...
func printHelp() {
	...
	if build.DetectBuilder() == "buildkit" {
		...
		fmt.Println("                                        Supported types: registry, inline, local, s3, azblob, gha")
		...
		fmt.Println("                                          type=s3,region=us-east-1,bucket=my-cache,mode=max")
		fmt.Println("                                          type=azblob,account_url=https://x.blob.core.windows.net,name=cache")
		fmt.Println("                                          type=gha")
		...
		fmt.Println("                                          type=s3,region=us-east-1,bucket=my-cache")
		fmt.Println("                                          type=gha")
	}
	fmt.Println("  --secret SPEC                         Mount a build-time secret (repeatable)")
	fmt.Println("                                        Consumed by RUN --mount=type=secret,id=... in the")
	fmt.Println("                                        Dockerfile. The value never lands in an image layer,")
	fmt.Println("                                        in build history, or in an exported cache.")
	fmt.Println("                                        Examples:")
	fmt.Println("                                          id=npmrc,src=/run/secrets/npmrc")
	fmt.Println("                                          id=npm_token,env=NPM_TOKEN")
	fmt.Println("                                        Note: this mounts a secret INTO a build. It is unrelated")
	fmt.Println("                                        to `rfscan --secrets`, which scans a built image for")
	...
	fmt.Printf("                                        Example: --timestamp=$(date +%%s)\n")
	fmt.Println("                                                 --timestamp=1609459200")
	fmt.Printf("                                                 --timestamp=$(git log -1 --format=%%ct)\n")
	fmt.Println()
	if build.DetectBuilder() == "buildkit" {
	...
	fmt.Printf("  export SOURCE_DATE_EPOCH=$(git log -1 --format=%%ct)\n")
	fmt.Println("  kimia --context=. \\")
	fmt.Println("         --destination=registry.io/myapp:v1 \\")
	fmt.Println("         --reproducible")
	fmt.Println()
	if build.DetectBuilder() == "buildkit" {
		...
		fmt.Println()
		fmt.Println("  # Build with S3 cache (credentials resolved from the environment,")
		fmt.Println("  # so pod-level IRSA works with no explicit keys)")
		fmt.Println("  kimia --context=. \\")
		fmt.Println("         --destination=registry.io/myapp:v1 \\")
		fmt.Println("         --cache \\")
		fmt.Println("         --import-cache type=s3,region=us-east-1,bucket=my-build-cache \\")
		fmt.Println("         --export-cache type=s3,region=us-east-1,bucket=my-build-cache,mode=max")
		fmt.Println()
	...
	fmt.Println("  # Build with a secret (credentials stay out of the image and cache)")
	fmt.Println("  # Dockerfile:")
	fmt.Println("  #   RUN --mount=type=secret,id=npmrc,target=/root/.npmrc npm install")
	fmt.Println("  kimia --context=. \\")
	fmt.Println("         --destination=registry.io/myapp:v1 \\")
	fmt.Println("         --secret id=npmrc,src=/run/secrets/npmrc")
	fmt.Println()
	fmt.Println("  # Same, sourcing the secret from an environment variable")
	fmt.Println("  # Dockerfile:")
	fmt.Println("  #   RUN --mount=type=secret,id=npm_token,env=NPM_TOKEN npm install")
	fmt.Println("  kimia --context=. \\")
	fmt.Println("         --destination=registry.io/myapp:v1 \\")
	fmt.Println("         --secret id=npm_token,env=NPM_TOKEN")
	fmt.Println()
