...
func printHelp() {
	...
	fmt.Println("  --storage-driver DRIVER               Storage driver: vfs (default), overlay")
	fmt.Println()
	...
	fmt.Println("  vfs       VFS driver (default) - Most compatible, slower performance")
	fmt.Println("  overlay   Overlay driver - Faster performance, requires fuse-overlayfs")
	fmt.Println()
	...
	fmt.Println()
	fmt.Println("  # Build with overlay storage driver for better performance")
	fmt.Println("  smithy --context=. --destination=registry.io/myapp:latest \\")
